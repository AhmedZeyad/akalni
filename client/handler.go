...
func (h ClientHandler) GetProfile(ctx *gin.Context) {
	...
	client, err := h.service.GetProfile(claims.ID)
	if err != nil {
...
func (h *ClientHandler) Login(c *gin.Context) {
	...
	slog.Info("login success", "client", client)
	shared.Respond(c, client, nil)
...
func (h *ClientHandler) Refresh(c *gin.Context) {
	...
	slog.Info("refresh token", "token", strToken)
...
func (ch *ClientHandler) SendOtp(c *gin.Context) {
	...
	if req.Email == "" {
		slog.Error("email required")
		appError.Error = errors.New(customErrors.VALIDATION_EMAIL_REQUIRED)
		shared.Respond(c, nil, appError)
		return
	}
...
func (ch *ClientHandler) VerifyOtp(c *gin.Context) {
	...
	clientID := c.MustGet("client").(middleware.ClientClaims).ID
	err = ch.service.VerifyEmail(clientID, req, ch.conf.OTPSalt, OTP_TYPE_EMAIL_VERIFICATION)
	if err != nil {
		appError.Error = err
...
func (ch *ClientHandler) VerifyUpdateEmail(c *gin.Context) {
	...
	clientID := c.MustGet("client").(middleware.ClientClaims).ID
	err = ch.service.VerifyEmail(clientID, req, ch.conf.OTPSalt, OTP_TYPE_EMAIL_UPDATE)
	if err != nil {
		appError.Error = err
		slog.Error("failed to verify email", "error", appError)
		shared.Respond(c, nil, appError)
