...
func (jwtservice *JTWSevice) TokenVerify(stringtToken string) (Claims, error) {
	...
	if !ok {
		return Claims{}, errors.New(customeErrors.AUTH_INVALID_CREDENTIALS)
	}
...
func (jwtservice *JTWSevice) RefreshTokenVerify(strToken string) (claims Claims, err error) {
	...
	if err != nil {
		if errors.Is(err, jwt.ErrTokenExpired) {
			return Claims{}, errors.New(customeErrors.AUTH_TOKEN_EXPIRED)
		}
		return Claims{}, err
	}
	if token == nil {
		return Claims{}, errors.New(customeErrors.AUTH_INVALID_TOKEN)
	}
	if !token.Valid {
		return Claims{}, errors.New(customeErrors.AUTH_INVALID_TOKEN)
	}
